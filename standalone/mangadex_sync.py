...
async function paste(id) {
  try {
    // Use server-side clipboard read to avoid pywebview permission issues
...
if __name__ == "__main__":
    import sys, threading, webbrowser, time
    ...
    try:
        ...
        print(f"\n  pywebview not installed — opening in browser.")
        print(f"  To get the native window:  pip install pywebview PyQt6 PyQt6-WebEngine qtpy\n")
        print(f"  MangaDex Exporter v2.0  —  http://localhost:{PORT}\n")
        ...
        print(f"\n  Could not open native window — falling back to browser.")
